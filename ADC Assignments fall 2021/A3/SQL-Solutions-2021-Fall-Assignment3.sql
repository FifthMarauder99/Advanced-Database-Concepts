...
where  w.cname = c.cname and w.salary <= 70000 and w.pid not in (select ps.pid from personskill ps where skill = 'Programming') or
       w.cname = c.cname and w.salary <= 70000 and w.pid not in (select ps.pid from personskill ps where skill = 'AI')) q;
...
(select c.cname, c.headquarter
 ...
 where  w.cname = c.cname and w.salary <= 70000 and 
        not exists (select 1
                    from   personskill ps 
                    where  w.pid = ps.pid and skill = 'AI'))) q;
...
(select q.ccname, q.headquarter
 ...
 from 
   (select c.cname as ccname, c.headquarter, w.*
    ...
    select c.*, w.*
    from   company c, worksfor w, personskill ps
    where  w.pid = ps.pid and ps.skill = 'AI') q)) q;
 ...
 from 
   (select c.cname as ccname, c.headquarter, w.*
    ...
    from   company c 
           ...
           join  (select * 
                  from   personskill 
                  where  skill = 'AI') ps on w.pid = ps.pid) q)) q;
...
-- Optimization begins
-- Remove out projection since the inside except is over the same
-- set of attributes as this out projection, i.e., the attributes cname, headquarter
